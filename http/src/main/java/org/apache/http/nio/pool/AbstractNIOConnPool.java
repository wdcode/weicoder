...
public abstract class AbstractNIOConnPool<T, C, E extends PoolEntry<T, C>> implements ConnPool<T, E>, ConnPoolControl<T> {
	 ...
	 */
	@Deprecated
	public AbstractNIOConnPool(final ConnectingIOReactor ioReactor, final NIOConnFactory<T, C> connFactory, final int defaultMaxPerRoute, final int maxTotal) {
		super();
	 ...
	 * @since 4.3
	 */
	public AbstractNIOConnPool(final ConnectingIOReactor ioReactor, final NIOConnFactory<T, C> connFactory, final SocketAddressResolver<T> addressResolver, final int defaultMaxPerRoute,
			final int maxTotal) {
		super();
		Args.notNull(ioReactor, "I/O reactor");
		Args.notNull(connFactory, "Connection factory");
	...
	public Future<E> lease(final T route, final Object state, final long connectTimeout, final TimeUnit timeUnit, final FutureCallback<E> callback) {
	 ...
	 * @since 4.3
	 */
	public Future<E> lease(final T route, final Object state, final long connectTimeout, final long leaseTimeout, final TimeUnit timeUnit, final FutureCallback<E> callback) {
		Args.notNull(route, "Route");
		Args.notNull(timeUnit, "Time unit");
		Asserts.check(!this.isShutDown.get(), "Connection pool shut down");
		final BasicFuture<E> future = new BasicFuture<E>(callback);
		final LeaseRequest<T, C, E> leaseRequest = new LeaseRequest<T, C, E>(route, state, connectTimeout >= 0 ? timeUnit.toMillis(connectTimeout) : -1,
	...
	private boolean processPendingRequest(final LeaseRequest<T, C, E> request) {
		...
		if (pool.getAllocatedCount() < maxPerRoute) {
			...
			final SessionRequest sessionRequest = this.ioReactor.connect(remoteAddress, localAddress, route, this.sessionRequestCallback);
	...
	public PoolStats getStats(final T route) {
		...
		try {
			...
			return new PoolStats(pool.getLeasedCount(), pendingCount + pool.getPendingCount(), pool.getAvailableCount(), getMax(route));
		} finally {
			this.lock.unlock();
		}
	}
	...
	/**
	 * Use {@link #enumLeased(org.apache.http.pool.PoolEntryCallback)} or {@link #enumAvailable(org.apache.http.pool.PoolEntryCallback)} instead.
	 *
	 * @deprecated (4.3.2)
	 */
	@Deprecated
