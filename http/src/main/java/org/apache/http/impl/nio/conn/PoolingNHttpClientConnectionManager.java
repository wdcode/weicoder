 ...
 * <p>
 * {@code PoolingNHttpClientConnectionManager} maintains a maximum limit of connection on a per route basis and in total. Per default this implementation will create no more than than 2 concurrent
 * connections per given route and no more 20 connections in total. For many real-world applications these limits may prove too constraining, especially if they use HTTP as a transport protocol for
 * their services. Connection limits, however, can be adjusted using {@link ConnPoolControl} methods.
 *
 * @since 4.0
 */
...
public class PoolingNHttpClientConnectionManager implements NHttpClientConnectionManager, ConnPoolControl<HttpRoute> {
	...
	private static Registry<SchemeIOSessionStrategy> getDefaultRegistry() {
		return RegistryBuilder.<SchemeIOSessionStrategy>create().register("http", NoopIOSessionStrategy.INSTANCE).register("https", SSLIOSessionStrategy.getDefaultStrategy()).build();
	}
	...
	public PoolingNHttpClientConnectionManager(final ConnectingIOReactor ioreactor, final Registry<SchemeIOSessionStrategy> iosessionFactoryRegistry) {
		this(ioreactor, null, iosessionFactoryRegistry, (DnsResolver) null);
	}

	public PoolingNHttpClientConnectionManager(final ConnectingIOReactor ioreactor, final NHttpConnectionFactory<ManagedNHttpClientConnection> connFactory, final DnsResolver dnsResolver) {
		this(ioreactor, connFactory, getDefaultRegistry(), dnsResolver);
	}

	public PoolingNHttpClientConnectionManager(final ConnectingIOReactor ioreactor, final NHttpConnectionFactory<ManagedNHttpClientConnection> connFactory,
	...
	public PoolingNHttpClientConnectionManager(final ConnectingIOReactor ioreactor, final NHttpConnectionFactory<ManagedNHttpClientConnection> connFactory,
			final Registry<SchemeIOSessionStrategy> iosessionFactoryRegistry, final SocketAddressResolver<HttpRoute> socketAddressResolver) {
		this(ioreactor, connFactory, iosessionFactoryRegistry, socketAddressResolver, -1, TimeUnit.MILLISECONDS);
	}

	public PoolingNHttpClientConnectionManager(final ConnectingIOReactor ioreactor, final NHttpConnectionFactory<ManagedNHttpClientConnection> connFactory,
			final Registry<SchemeIOSessionStrategy> iosessionFactoryRegistry, final SchemePortResolver schemePortResolver, final DnsResolver dnsResolver, final long timeToLive, final TimeUnit tunit) {
		this(ioreactor, connFactory, iosessionFactoryRegistry, new InternalAddressResolver(schemePortResolver, dnsResolver), timeToLive, tunit);
	}

	public PoolingNHttpClientConnectionManager(final ConnectingIOReactor ioreactor, final NHttpConnectionFactory<ManagedNHttpClientConnection> connFactory,
			final Registry<SchemeIOSessionStrategy> iosessionFactoryRegistry, final SocketAddressResolver<HttpRoute> socketAddressResolver, final long timeToLive, final TimeUnit tunit) {
		super();
		...
		this.pool = new CPool(ioreactor, new InternalConnectionFactory(this.configData, connFactory), socketAddressResolver, 2, 20, timeToLive, tunit != null ? tunit : TimeUnit.MILLISECONDS);
		this.iosessionFactoryRegistry = iosessionFactoryRegistry;
	}

	PoolingNHttpClientConnectionManager(final ConnectingIOReactor ioreactor, final CPool pool, final Registry<SchemeIOSessionStrategy> iosessionFactoryRegistry) {
		super();
		this.ioreactor = ioreactor;
		this.configData = new ConfigData();
		this.pool = pool;
...
//		}
	...
	public Future<NHttpClientConnection> requestConnection(final HttpRoute route, final Object state, final long connectTimeout, final long leaseTimeout, final TimeUnit tunit,
		...
		final Future<CPoolEntry> leaseFuture = this.pool.lease(route, state, connectTimeout, leaseTimeout, tunit != null ? tunit : TimeUnit.MILLISECONDS, new FutureCallback<CPoolEntry>() {
	...
	private Lookup<SchemeIOSessionStrategy> getIOSessionFactoryRegistry(final HttpContext context) {
		...
		Lookup<SchemeIOSessionStrategy> reg = (Lookup<SchemeIOSessionStrategy>) context.getAttribute(IOSESSION_FACTORY_REGISTRY);
		if (reg == null) {
	...
	@Override
	public void startRoute(final NHttpClientConnection managedConn, final HttpRoute route, final HttpContext context) throws IOException {
		Args.notNull(managedConn, "Managed connection");
		Args.notNull(route, "HTTP route");
		final HttpHost host;
		if (route.getProxyHost() != null) {
	...
	@Override
	public void upgrade(final NHttpClientConnection managedConn, final HttpRoute route, final HttpContext context) throws IOException {
		Args.notNull(managedConn, "Managed connection");
		Args.notNull(route, "HTTP route");
	...
	@Override
	public void routeComplete(final NHttpClientConnection managedConn, final HttpRoute route, final HttpContext context) {
		Args.notNull(managedConn, "Managed connection");
		Args.notNull(route, "HTTP route");
		synchronized (managedConn) {
			final CPoolEntry entry = CPoolProxy.getPoolEntry(managedConn);
	...
	static class InternalConnectionFactory implements NIOConnFactory<HttpRoute, ManagedNHttpClientConnection> {
		...
		InternalConnectionFactory(final ConfigData configData, final NHttpConnectionFactory<ManagedNHttpClientConnection> connFactory) {
			super();
		...
		@Override
		public ManagedNHttpClientConnection create(final HttpRoute route, final IOSession iosession) throws IOException {
