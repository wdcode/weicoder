...
public final class HibernateDao implements Dao {
	...
	@Override
	@SuppressWarnings("unchecked")
	public <E> List<E> query(Class<?> entityClass, final String sql, final List<Object> values, final int firstResult,
			final int maxResults) {
		return execute(entityClass,
				(Session session) -> setParameter(session.createNativeQuery(sql), values, firstResult, maxResults)
						.getResultList());
	}
