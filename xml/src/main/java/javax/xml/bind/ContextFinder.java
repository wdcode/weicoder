...
class ContextFinder {
    ...
    private static ServiceLoaderUtil.ExceptionHandler<Exception> EXCEPTION_HANDLER =
            new ServiceLoaderUtil.ExceptionHandler<Exception>() {
                @Override
                public Exception createException(Throwable throwable, String message) {
     ...
     */
    private static Throwable handleInvocationTargetException(InvocationTargetException x) throws Exception {
     ...
     */
    private static Exception handleClassCastException(Class originalType, Class targetType) {
        final URL targetTypeURL = which(targetType);

        return new Exception(Messages.format(Messages.ILLEGAL_CAST,
    ...
    static JAXBContext newInstance(String contextPath,
        ...
        try {
            ...
            throw new Exception(Messages.format(Messages.DEFAULT_PROVIDER_NOT_FOUND), x);

        }   catch (Exception x) {
            // can't catch Exception because the method is hidden behind
            ...
            throw new Exception(Messages.format(Messages.COULD_NOT_INSTANTIATE, className, x), x);
        }
    }
    ...
    static JAXBContext newInstance(String contextPath,
        ...
        try {
            ...
            // otherwise, wrap with a Exception
            Throwable e = handleInvocationTargetException(x);
            throw new Exception(Messages.format(Messages.COULD_NOT_INSTANTIATE, spFactory, e), e);
            ...
            // createContext() are reported as JAXBExceptions - just re-throw it
            // some other type of exception - just wrap it
            throw new Exception(Messages.format(Messages.COULD_NOT_INSTANTIATE, spFactory, x), x);
        }
    }

    private static Object instantiateProviderIfNecessary(final Class<?> implClass) throws Exception {
        try {
            ...
            throw new Exception(Messages.format(Messages.COULD_NOT_INSTANTIATE, implClass, e), e);
        }
    }
     ...
     */
    static JAXBContext newInstance(Class[] classes, Map properties, String className) throws Exception {

        Class spi;
        try {
            spi = ServiceLoaderUtil.safeLoadClass(className, PLATFORM_DEFAULT_FACTORY_CLASS, getContextClassLoader());
        } catch (ClassNotFoundException e) {
            throw new Exception(Messages.format(Messages.DEFAULT_PROVIDER_NOT_FOUND), e);
        }
    ...
    static JAXBContext newInstance(Class[] classes,
                                   Map properties,
                                   Class spFactory) throws Exception {
        try {
    ...
    static JAXBContext find(String factoryId,
        ...
        if (contextPath == null || contextPath.isEmpty()) {
            ...
            throw new Exception(Messages.format(Messages.NO_PACKAGE_IN_CONTEXTPATH));
        }
    ...
    static JAXBContext find(Class<?>[] classes, Map<String, ?> properties) throws Exception {
    ...
    private static String classNameFromPackageProperties(URL packagePropertiesUrl,
                                                         String ... factoryIds) throws Exception {
        ...
        throw new Exception(Messages.format(Messages.MISSING_PROPERTY, packageName, factoryIds[0]));
    }

    private static String classNameFromSystemProperties() throws Exception {
    ...
    private static Properties loadJAXBProperties(URL url) throws Exception {
    ...
    private static String jaxbProperties(String contextPath, ClassLoader classLoader, String factoryId) throws Exception {
    ...
    private static String jaxbProperties(Class[] classesFromContextPath, String factoryId) throws Exception {
