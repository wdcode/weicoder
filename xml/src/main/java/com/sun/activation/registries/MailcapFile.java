...
public class MailcapFile {
	...
	/**
	 * A Map indexed by MIME type (string) that references a Map of commands for each type. The comand Map is indexed by the command name and references a List of class names (strings) for each
	 * command.
	 */
	 ...
	 * <p>
	 * <strong>Semantics:</strong> First check for the literal mime type, if that fails looks for wildcard <type>/\* and return that. Return the list of all that hit.
	 */
	...
	/**
	 * appendToMailcap: Append to this Mailcap DB, use the mailcap format: Comment == "# <i>comment string</i> Entry == "mimetype; javabeanclass
	 * <nl>
	 ...
	 */
	protected void parseLine(String mailcapEntry) throws Exception, IOException {
	...
	protected static void reportParseError(int expectedToken, int actualToken, String actualTokenValue) throws Exception {
		throw new Exception(
				"Encountered a " + MailcapTokenizer.nameForToken(actualToken) + " token (" + actualTokenValue + ") while expecting a " + MailcapTokenizer.nameForToken(expectedToken) + " token.");
	}

	protected static void reportParseError(int expectedToken, int otherExpectedToken, int actualToken, String actualTokenValue) throws Exception {
		throw new Exception("Encountered a " + MailcapTokenizer.nameForToken(actualToken) + " token (" + actualTokenValue + ") while expecting a " + MailcapTokenizer.nameForToken(expectedToken)
				+ " or a " + MailcapTokenizer.nameForToken(otherExpectedToken) + " token.");
	}

	protected static void reportParseError(int expectedToken, int otherExpectedToken, int anotherExpectedToken, int actualToken, String actualTokenValue) throws Exception {
		if (LogSupport.isLoggable())
			LogSupport.log("PARSE ERROR: " + "Encountered a " + MailcapTokenizer.nameForToken(actualToken) + " token (" + actualTokenValue + ") while expecting a "
					+ MailcapTokenizer.nameForToken(expectedToken) + ", a " + MailcapTokenizer.nameForToken(otherExpectedToken) + ", or a " + MailcapTokenizer.nameForToken(anotherExpectedToken)
					+ " token.");
		throw new Exception("Encountered a " + MailcapTokenizer.nameForToken(actualToken) + " token (" + actualTokenValue + ") while expecting a " + MailcapTokenizer.nameForToken(expectedToken)
				+ ", a " + MailcapTokenizer.nameForToken(otherExpectedToken) + ", or a " + MailcapTokenizer.nameForToken(anotherExpectedToken) + " token.");
	}

	/**
	 * for debugging public static void main(String[] args) throws Exception { Map masterHash = new HashMap(); for (int i = 0; i < args.length; ++i) { System.out.println("Entry " + i + ": " +
	 * args[i]); parseLine(args[i], masterHash); } Enumeration types = masterHash.keys(); while (types.hasMoreElements()) { String key = (String)types.nextElement(); System.out.println("MIME Type: " +
	 * key); Map commandHash = (Map)masterHash.get(key); Enumeration commands = commandHash.keys(); while (commands.hasMoreElements()) { String command = (String)commands.nextElement();
	 * System.out.println(" Command: " + command); Vector classes = (Vector)commandHash.get(command); for (int i = 0; i < classes.size(); ++i) { System.out.println(" Class: " +
