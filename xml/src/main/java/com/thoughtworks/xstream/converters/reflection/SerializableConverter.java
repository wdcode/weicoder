...
public class SerializableConverter extends AbstractReflectionConverter {
	...
	public void doMarshal(final Object source, final HierarchicalStreamWriter writer, final MarshallingContext context) {
		...
		CustomObjectOutputStream.StreamCallback callback = new CustomObjectOutputStream.StreamCallback() {
			...
			public void writeFieldsToStream(Map fields) {
				...
				for (Iterator iterator = fields.keySet().iterator(); iterator.hasNext();) {
					...
					if (field == null) {
						throw new RuntimeException(value.getClass().getName() + name);
					}
					if (value != null) {
						ExtendedHierarchicalStreamWriterHelper.startNode(writer, mapper.serializedMember(source.getClass(), name), value.getClass());
			...
			public void defaultWriteObject() {
				...
				for (int i = 0; i < fields.length; i++) {
					...
					if (value != null) {
						...
						ExtendedHierarchicalStreamWriterHelper.startNode(writer, mapper.serializedMember(source.getClass(), field.getName()), actualType);
		...
		try {
			...
			throw new RuntimeException("Cannot write defaults", e);
		}
	}
	...
	public Object doUnmarshal(final Object result, final HierarchicalStreamReader reader, final UnmarshallingContext context) {
		...
		if (attributeName != null && !ATTRIBUTE_VALUE_CUSTOM.equals(reader.getAttribute(attributeName))) {
			throw new RuntimeException("Cannot deserialize object with new readObject()/writeObject() methods");
		}
		...
		CustomObjectInputStream.StreamCallback callback = new CustomObjectInputStream.StreamCallback() {
				...
				if (reader.getNodeName().equals(ELEMENT_FIELDS)) {
					...
					while (reader.hasMoreChildren()) {
						...
						if (!reader.getNodeName().equals(ELEMENT_FIELD)) {
							throw new RuntimeException("Expected <" + ELEMENT_FIELD + "/> element inside <" + ELEMENT_FIELD + "/>");
						}
					...
					while (reader.hasMoreChildren()) {
						...
						if (mapper.shouldSerializeMember(currentType[0], name)) {
							...
							if (classAttribute != null) {
								...
								ObjectStreamField field = objectStreamClass.getField(name);
								if (field == null) {
									throw new RuntimeException(currentType[0].getName() + name);
								}
								type = field.getType();
							}
							Object value = context.convertAnother(result, type);
					...
					throw new RuntimeException("Expected <" + ELEMENT_FIELDS + "/> or <" + ELEMENT_DEFAULT + "/> element when calling ObjectInputStream.readFields()");
				}
				reader.moveUp();
				return fields;
			}
			...
			public void defaultReadObject() {
				...
				if (!reader.getNodeName().equals(ELEMENT_DEFAULT)) {
					throw new RuntimeException("Expected <" + ELEMENT_DEFAULT + "/> element in readObject() stream");
				}
				while (reader.hasMoreChildren()) {
					reader.moveDown();
				...
				context.addCompletionCallback(new Runnable() {
					public void run() {
						try {
							...
							throw new RuntimeException("Cannot validate object", e);
						}
					}
				}, priority);
			}
		...
		while (reader.hasMoreChildren()) {
			...
			if (nodeName.equals(ELEMENT_UNSERIALIZABLE_PARENTS)) {
				...
				if (serializationMembers.supportsReadObject(currentType[0], false)) {
					CustomObjectInputStream objectInputStream = CustomObjectInputStream.getInstance(context, callback, classLoaderReference);
					...
					try {
						...
						throw new RuntimeException("Cannot read defaults", e);
					}
				}
			}
			reader.moveUp();
