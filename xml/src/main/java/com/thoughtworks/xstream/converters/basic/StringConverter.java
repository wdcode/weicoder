 ...
 * <p>
 * Well ok, it doesn't <i>actually</i> do any conversion. The converter uses by default a map with weak references to reuse instances of strings that do not exceed a length limit. This limit is by
 * default 38 characters to cache typical strings containing UUIDs. Only shorter strings are typically repeated more often in XML values.
 * </p>
 * 
 * @author Joe Walnes
...
public class StringConverter extends AbstractSingleValueConverter {
	...
	/**
	 * A Map to store strings as long as needed to map similar strings onto the same instance and conserve memory. The map can be set from the outside during construction, so it can be a LRU map or a
	 * weak map, synchronised or not.
	 */
