 ...
 * Writing in a wrong state will throw a {@link RuntimeException}, that wraps either an
...
public class StatefulWriter extends WriterWrapper {
    ...
    private void startNodeCommon() {
        ...
        if (state == STATE_VALUE) {
            ...
            throw new RuntimeException(new IllegalStateException("Opening node after writing text"));
        }
    ...
    public void addAttribute(String name, String value) {
        ...
        if (state != STATE_NODE_START) {
            throw new RuntimeException(new IllegalStateException("Writing attribute '"
                    + name
    ...
    public void endNode() {
        checkClosed();
        if (balance-- == 0) {
            throw new RuntimeException(new IllegalStateException("Unbalanced node"));
        }
    ...
    public void close() {
        if (state != STATE_NODE_END && state != STATE_OPEN) {
            ...
            // throw new RuntimeException(new IllegalStateException("Closing with unbalanced tag"));
        }
        state = STATE_CLOSED;
        super.close();
    }
    ...
    private void checkClosed() {
        if (state == STATE_CLOSED) {
            throw new RuntimeException(new IOException("Writing on a closed stream"));
        }
    }
