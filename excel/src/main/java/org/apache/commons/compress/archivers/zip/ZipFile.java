 ...
 * <p>
 * This class adds support for file name encodings other than UTF-8 (which is required to work on ZIP files created by native zip tools and is able to skip a preamble like the one found in self
 * extracting archives. Furthermore it returns instances of <code>org.apache.commons.compress.archivers.zip.ZipArchiveEntry</code> instead of <code>java.util.zip.ZipEntry</code>.
 * </p>
 * <p>
 * It doesn't extend <code>java.util.zip.ZipFile</code> as it would have to reimplement all methods anyway. Like <code>java.util.ZipFile</code>, it uses SeekableByteChannel under the covers and
 * supports compressed and uncompressed entries. As of Apache Commons Compress 1.3 it also transparently supports Zip64 extensions and thus individual entries and archives larger than 4 GB or with
...
public class ZipFile implements Closeable {
	 ...
	 * <p>
	 * For a list of possible values see <a href="http://java.sun.com/j2se/1.5.0/docs/guide/intl/encoding.doc.html">http://java.sun.com/j2se/1.5.0/docs/guide/intl/encoding.doc.html</a>. Defaults to
	 * UTF-8.
	 * </p>
	 */
	 ...
	 * Opens the given file for reading, assuming the specified encoding for file names.
	 * <p>
	 * By default the central directory record and all local file headers of the archive will be read immediately which may take a considerable amount of time when the archive is big. The
	 * {@code ignoreLocalFileHeader} parameter can be set to {@code true} which restricts parsing to the central directory. Unfortunately the local file header may contain information not present
	 * inside of the central directory which will not be available when the argument is set to {@code true}. This includes the content of the Unicode extra field, so setting {@code
	 * ignoreLocalFileHeader} to {@code true} means {@code useUnicodeExtraFields} will be ignored effectively. Also {@link #getRawInputStream} is always going to return {@code null} if
	 * {@code ignoreLocalFileHeader} is {@code
	 * true}.
	 * </p>
	 *
	 * @param  f                     the archive.
	...
	private ZipFile(final SeekableByteChannel channel, final String archiveName, final String encoding, final boolean useUnicodeExtraFields, final boolean closeOnError,
	 ...
	 *
	 * @param noUTF8Flag map used to collect entries that don't have their UTF-8 flag set and whose name will be set by data read from the local file header later. The current entry may be added to
	 *                   this map.
	 */
	 ...
	 * <p>
	 * Ensures the Zip64 extra either knows both compressed and uncompressed size or neither of both as the internal logic in ExtraFieldUtils forces the field to create local header data even if they
	 * are never used - and here a field with only one size would be invalid.
	 * </p>
	 */
	 ...
	 */
	private static final int CFD_LOCATOR_OFFSET = /* end of central dir signature */ WORD/* number of this disk */ + SHORT
	/* number of the disk with the */
	/* start of the central directory */ + SHORT
	/* total number of entries in */
	/* the central dir on this disk */ + SHORT
	...
	/**
	 * Offset of the field that holds the location of the "Zip64 end of central directory record" inside the "Zip64 end of central directory locator" relative to the start of the "Zip64 end of central
	 * directory locator".
	 */
	...
	/**
	 * Offset of the field that holds the location of the first central directory entry inside the "Zip64 end of central directory record" relative to the start of the "Zip64 end of central directory
	 * record".
	 */
	...
	/**
	 * Searches for either the &quot;Zip64 end of central directory locator&quot; or the &quot;End of central dir record&quot;, parses it and positions the stream at the first central directory
	 * record.
	 */
	...
	/**
	 * Parses the &quot;Zip64 end of central directory locator&quot;, finds the &quot;Zip64 end of central directory record&quot; using the parsed information, parses that and positions the stream at
	 * the first central directory record. Expects stream to be positioned right behind the &quot;Zip64 end of central directory locator&quot;'s signature.
	 */
	...
	/**
	 * Parses the &quot;End of central dir record&quot; and positions the stream at the first central directory record. Expects stream to be positioned at the beginning of the &quot;End of central dir
	 * record&quot;.
	 */
	...
	/**
	 * Lock-free implementation of BoundedInputStream. The implementation uses positioned reads on the underlying archive file channel and therefore performs significantly faster in concurrent
	 * environment.
	 */
