package com.weicoder.frame.dao.hibernate;
...
public final class HibernateDao implements Dao {
	 ...
	 * @return 数据列表
	 */
	public <E> List<E> search(final Class<E> entityClass, final String property, final Object value,
			final int firstResult, final int maxResults) {
		return execute(entityClass, new Callback<List<E>>() {
			public List<E> callback(Session session) {
				return search.search(session, entityClass, property, value, firstResult,
						maxResults);
			}
		});
	}
	 ...
	 * @return 数据列表
	 */
	public <E> List<E> in(Class<E> entityClass, String property, List<?> values, int firstResult,
			int maxResults) {
		return queryCriteria(entityClass,
	...
	public <E> List<E> query(Class<?> entityClass, final String sql, final List<Object> values,
		...
		return execute(entityClass, new Callback<List<E>>() {
			@SuppressWarnings("unchecked")
			public List<E> callback(Session session) {
				return setParameter(session.createNativeQuery(sql), values, firstResult, maxResults)
						.list();
			}
		});
	}
	...
	private int count(Class<?> entityClass, final DetachedCriteria criteria) {
		return execute(entityClass, new Callback<Integer>() {
			public Integer callback(Session session) {
				return Conversion.toInt(criteria.getExecutableCriteria(session)
