...
public final class HibernateSearch {
	...
	public <E> List<E> search(Session session, Class<E> entityClass, final String property,
			final Object value, final int firstResult, final int maxResults) {
		...
		// 得到search query
		org.apache.lucene.search.Query query = queryBuilder.keyword().onField(property)
	...
	public <E> List<E> search(Session session, E entity, int firstResult, int maxResults) {
		...
		FullTextQuery fullTextQuery = fullTextSession.createFullTextQuery(query, entity.getClass())
				.setCriteriaQuery(DetachedCriteria.forClass(entity.getClass())
						.getExecutableCriteria(fullTextSession).add(Example.create(entity)));
		// 开始结果大于等于0
		if (firstResult >= 0) {
			fullTextQuery.setFirstResult(firstResult);
		}
