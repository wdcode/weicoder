...
public final class HttpUtil {
	...
	public static String toUrl(String url, Map<String, String> parameters) {
		return U.S.add(url, "?", toParameters(parameters));
	}
	...
	public static String getContentType(String url) {
		// 如果有?把?去掉 并获得后缀
		String suf = U.S.subStringLast(U.S.subStringEnd(url, C.S.QUESTION_MARK), C.S.POINT).toLowerCase();
		// 判断是什么类型的文件
		if (C.F.SUFFIX_JS.equals(suf)) {
			return C.H.CONTENT_TYPE_JS;
		} else if (C.F.SUFFIX_CSS.equals(suf)) {
			return C.H.CONTENT_TYPE_CSS;
		} else if (C.F.SUFFIX_HTML.equals(suf)) {
			return C.H.CONTENT_TYPE_HTML;
		} else if (C.F.SUFFIX_TXT.equals(suf)) {
			return C.H.CONTENT_TYPE_TXT;
		} else if (C.F.SUFFIX_GIF.equals(suf)) {
			return C.H.CONTENT_TYPE_GIF;
		} else if (C.F.SUFFIX_JPG.equals(suf) || C.F.SUFFIX_JPEG.equals(suf)) {
			return C.H.CONTENT_TYPE_JPEG;
		} else if (C.F.SUFFIX_PNG.equals(suf)) {
			return C.H.CONTENT_TYPE_PNG;
		} else {
			return C.S.EMPTY;
		}
	}
	...
	public static void setExpiresHeader(HttpServletResponse response, long expiresSeconds) {
		// Http 1.0 header
		response.setDateHeader(C.H.HEADER_KEY_EXPIRES, System.currentTimeMillis() + expiresSeconds * C.D.TIME_SECOND);
		// Http 1.1 header
		response.setHeader(C.H.HEADER_KEY_CACHE_CONTROL, "max-age=" + expiresSeconds);
	}
	...
	public static void setNoCacheHeader(HttpServletResponse response) {
		// Http 1.0 header
		response.setDateHeader(C.H.HEADER_KEY_EXPIRES, 0);
		// Http 1.1 header
	...
	public static boolean checkIfModifiedSince(HttpServletRequest request, HttpServletResponse response, long lastModified) {
		...
		// 判断时间
		if ((ifModifiedSince != -1) && (lastModified < ifModifiedSince + C.D.TIME_SECOND)) {
			// 设置没修改
	...
	public static boolean checkIfNoneMatchEtag(HttpServletRequest request, HttpServletResponse response, String etag) {
		...
		if (U.E.isNotEmpty(headerValue)) {
			...
			if (!"*".equals(headerValue)) {
				// 声明StringTokenizer
				StringTokenizer commaTokenizer = new StringTokenizer(headerValue, C.S.COMMA);
				// 循环处理
	...
	public static boolean checkAccetptGzip(HttpServletRequest request) {
		...
		return U.S.contains(acceptEncoding, "gzip");
	}
