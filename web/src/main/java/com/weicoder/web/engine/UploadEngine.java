...
public final class UploadEngine {
	...
	public static String upload(HttpServletRequest request, byte[] file, String fileName, String path) {
		...
		// 上传文件
		String fn = UploadParams.UPLOAD_PATH + name;
		// 文件是否存在
		if (FileUtil.exists(fn)) {
			...
			while (FileUtil.exists(fn)) {
				...
				if (Hex.encode(Digest.md5(FileUtil.read(fn))).equals(md5)) {
					...
					// 文件不同 获得新文件名
					name = UploadParams.UPLOAD_PATH + getFileName((DateUtil.getTime() % 100) + StringConstants.UNDERLINE + fileName, path);
				}
			}
		}
		// 文件不存在写文件
		if (!FileUtil.exists(fn)) {
			...
			// 是否开启图片压缩 并且是图片
			if (UploadParams.UPLOAD_IMAGE_COMPRESS_POWER) {
				// 获得文件
