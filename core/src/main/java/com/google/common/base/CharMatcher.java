...
/**
 * Determines a true or false value for any Java {@code char} value, just as {@link Predicate} does for any {@link Object}. Also offers basic text processing methods based on this function.
 ...
 * <p>
 * <b>Warning:</b> This class deals only with {@code char} values, that is, <a href="http://www.unicode.org/glossary/#BMP_character">BMP characters</a>. It does not understand
 * <a href="http://www.unicode.org/glossary/#supplementary_code_point">supplementary Unicode code points</a> in the range {@code 0x10000} to {@code 0x10FFFF} which includes the majority of assigned
 * characters, including important CJK characters and emoji.
 * <p>
 * Supplementary characters are <a href="https://docs.oracle.com/javase/8/docs/api/java/lang/Character.html#supplementary">encoded into a {@code String} using surrogate pairs</a>, and a
 * {@code CharMatcher} treats these just as two separate characters. {@link #countIn} counts each supplementary character as 2 {@code char}s.
 * <p>
 * For up-to-date Unicode character properties (digit, letter, etc.) and support for supplementary code points, use ICU4J UCharacter and UnicodeSet (freeze() after building). For basic text processing
 ...
 * </pre>
 * <p>
 * See the Guava User Guide article on <a href="https://github.com/google/guava/wiki/StringsExplained#charmatcher">{@code CharMatcher} </a>.
 *
...
public abstract class CharMatcher implements Predicate<Character> {
	/*
	 * N777777777NO N7777777777777N M777777777777777N $N877777777D77777M N M77777777ONND777M MN777777777NN D777 N7ZN777777777NN ~M7778 N777777777777MMNN88777N N777777777777MNZZZ7777O
	 * DZN7777O77777777777777 N7OONND7777777D77777N 8$M++++?N???$77777$ M7++++N+M77777777N N77O777777777777$ M DNNM$$$$777777N D N$N:=N$777N7777M NZ 77Z::::N777777777 ODZZZ 77N::::::N77777777M NNZZZ$
	 * $777:::::::77777777MN ZM8ZZZZZ 777M::::::Z7777777Z77 N++ZZZZNN 7777M:::::M7777777$777M $++IZZZZM M777$:::::N777777$M7777M +++++ZZZDN NN$::::::7777$$M777777N N+++ZZZZNZ N::::::N:7$O:77777777
	 * N++++ZZZZN M::::::::::::N77777777+ +?+++++ZZZM 8::::::::::::D77777777M O+++++ZZ ::::::::::::M777777777N O+?D M:::::::::::M77777777778 77= D=::::::::::N7777777777N 777 INN===::::::=77777777777N
	 * I777N ?777N========N7777777777787M N7777 77777$D======N77777777777N777N? N777777 I77777$$$N7===M$$77777777$77777777$MMZ77777777N $$$$$$$$$$$NIZN$$$$$$$$$M$$7777777777777777ON M$$$$$$$$M
	 * M$$$$$$$$N=N$$$$7777777$$$ND O77Z$$$$$$$ M$$$$$$$$MNI==$DNNNNM=~N 7 :N MNN$$$$M$ $$$777$8 8D8I NMM.:7O 777777778 7777777MN M NO .7: M : M 8
	 */
	 ...
	 * <a href="http://unicode.org/cldr/utility/list-unicodeset.jsp?a=%5Cp%7Bwhitespace%7D">here</a>. This is not the same definition used by other Java APIs. (See a
	 * <a href="https://goo.gl/Y6SLWx">comparison of several definitions of "whitespace"</a>.)
	 * <p>
	 * All Unicode White_Space characters are on the BMP and thus supported by this API.
	 * <p>
	 * <b>Note:</b> as the Unicode definition evolves, we will modify this matcher to keep it up to date.
	 *
	...
	/**
	 * Determines whether a character is a breaking whitespace (that is, a whitespace which can be interpreted as a break between words for formatting purposes). See {@link #whitespace()} for a
	...
	/**
	 * Returns a {@code char} matcher that matches any BMP character present in the given character sequence. Returns a bogus matcher if the sequence contains supplementary characters.
	 */
	...
	/**
	 * Returns a {@code char} matcher that matches any BMP character not present in the given character sequence. Returns a bogus matcher if the sequence contains supplementary characters.
	 */
	...
	/**
	 * Returns a {@code char} matcher that matches any character in a given BMP range (both endpoints are inclusive). For example, to match any lowercase letter of the English alphabet, use {@code
	...
	/**
	 * Returns a {@code char} matcher functionally equivalent to this one, but which may be faster to query than the original; your mileage may vary. Precomputation takes time and is likely to be
	 * worthwhile only if the precomputed matcher is queried many thousands of times.
	 * <p>
	 * This method has no effect (returns {@code this}) when called in GWT: it's unclear whether a precomputed matcher is faster, but it certainly consumes more memory, which doesn't seem like a
	 ...
	 * <p>
	 * This implementation tries to be smart in a number of ways. It recognizes cases where the negation is cheaper to precompute than the matcher itself; it tries to build small hash tables for
	 * matchers that only match a few characters, and so on. In the worst-case scenario, it constructs an eight-kilobyte bit array and queries that. In many situations this produces a matcher which is
	 * faster to query than the original.
	 */
	 ...
	 * <p>
	 * The default implementation iterates over the sequence, invoking {@link #matches} for each character, until this returns {@code true} or the end is reached.
	 *
	 ...
	 * <p>
	 * The default implementation iterates over the sequence, invoking {@link #matches} for each character, until this returns {@code false} or the end is reached.
	 *
	 * @param  sequence the character sequence to examine, possibly empty
	 ...
	 * <p>
	 * The default implementation iterates over the sequence in forward order calling {@link #matches} for each character.
	 *
	 ...
	 * <p>
	 * The default implementation iterates over the sequence in reverse order calling {@link #matches} for each character.
	 *
	 ...
	 * <p>
	 * Counts 2 per supplementary character, such as for {@link #whitespace}().{@link #negate}().
	 */
	 ...
	 * ... returns {@code "rodor"}.
	 * <p>
	 * The default implementation uses {@link #indexIn(CharSequence)} to find the first matching character, then iterates the remainder of the sequence calling {@link #matches(char)} for each
	 * character.
	 *
	 ...
	 * ... returns {@code "yoohoo"}.
	 * <p>
	 * <b>Note:</b> If the replacement is a fixed string with only one character, you are better off calling {@link #replaceFrom(CharSequence, char)} directly.
	 *
	 * @param  sequence    the character sequence to replace matching characters in
	...
	/**
	 * Collapses groups of matching characters exactly as {@link #collapseFrom} does, except that groups of matching BMP characters at the start or end of the sequence are removed without replacement.
	 */
