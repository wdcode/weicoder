...
//			builder.threadsAllowedToBlockForConnectionMultiplier(100);
			...
			if (U.E.isNotEmpty(MongoParams.getUser(key)))
				credential = MongoCredential.createScramSha1Credential(MongoParams.getUser(key), "admin",
						MongoParams.getPassword(key).toCharArray());
			// 实例化客户端
			if (credential == null)
				client = new MongoClient(new ServerAddress(MongoParams.getHost(key), MongoParams.getPort(key)),
						builder.build());
			else
				client = new MongoClient(new ServerAddress(MongoParams.getHost(key), MongoParams.getPort(key)),
	...
	public List<Map<String, Object>> query(String name, Map<String, Object> query, int start, int end) {
		...
		FindIterable<Document> iterable = getCollection(name)
				.find(U.E.isEmpty(query) ? new BasicDBObject() : new BasicDBObject(query));
		// 设置游标开始位置
		iterable.skip(start);
		// 设置限定数量
	...
	@Override
	public void insert(String name, Map<String, Object> data) {
		getCollection(name).insertOne(new Document(data));
	}
	...
	public void insert(String name, Map<String, Object>... data) {
		// 声明Document列表
		List<Document> documents = Lists.newList(data.length);
		// 循环map数组
