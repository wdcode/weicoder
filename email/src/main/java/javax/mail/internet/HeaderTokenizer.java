...
public class HeaderTokenizer {
	...
	public static class Token {
		...
		/**
		 * Return the type of the token. If the token represents a delimiter or a control character, the type is that character itself, converted to an integer. Otherwise, it's value is one of the
		 * following:
		 * <ul>
		 * <li><code>ATOM</code> A sequence of ASCII characters delimited by either SPACE, CTL, "(", &lt;"&gt; or the specified SPECIALS
		...
		/**
		 * Returns the value of the token just read. When the current token is a quoted string, this field contains the body of the string, without the quotes. When the current token is a comment,
	 ...
	 * @param delimiters   Set of delimiter characters to be used to delimit ATOMS. These are usually <code>RFC822</code> or <code>MIME</code>
	...
	/**
	 * Parses the next token from this String. If endOfAtom is not NUL, the token extends until the endOfAtom character is seen, or to the end of the header. This method is useful when parsing headers
	 * that don't obey the MIME specification, e.g., by failing to quote parameter values that contain spaces.
	 *
	...
	/**
	 * Parses the next token from this String. endOfAtom is handled as above. If keepEscapes is true, any backslash escapes are preserved in the returned string. This method is useful when parsing
	 * headers that don't obey the MIME specification, e.g., by failing to escape backslashes in the filename parameter.
	 *
	 * @param     endOfAtom      if not NUL, character marking end of token
	 ...
	 *
	 * @return String rest of header. null is returned if we are already at end of header
	 */
	...
	/*
	 * Return the next token starting from 'currentPos'. After the parse, 'currentPos' is updated to point to the start of the next token.
	 */
