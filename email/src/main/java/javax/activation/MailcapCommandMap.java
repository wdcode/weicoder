...
/**
 * MailcapCommandMap extends the CommandMap abstract class. It implements a CommandMap whose configuration is based on mailcap files (<A HREF="http://www.ietf.org/rfc/rfc1524.txt">RFC 1524</A>). The
 * MailcapCommandMap can be configured both programmatically and via configuration files.
 * <p>
 * <b>Mailcap file search order:</b>
 * <p>
 * The MailcapCommandMap looks in various places in the user's system for mailcap file entries. When requests are made to search for commands in the MailcapCommandMap, it searches mailcap files in the
 * following order:
 * <ol>
 ...
 * <li>The file or resource named <code>META-INF/mailcap.default</code> (usually found only in the <code>activation.jar</code> file).
 * </ol>
 * <p>
 * (The current implementation looks for the <code>mailcap</code> file in the Java runtime in the directory <code><i>java.home</i>/conf</code> if it exists, and otherwise in the directory
 * <code><i>java.home</i>/lib</code>, where <i>java.home</i> is the value of the "java.home" System property. Note that the "conf" directory was introduced in JDK 9.)
 * <p>
 * <b>Mailcap file format:</b>
 * <p>
 * Mailcap files must conform to the mailcap file specification (RFC 1524, <i>A User Agent Configuration Mechanism For Multimedia Mail Format Information</i>). The file format consists of entries
 * corresponding to particular MIME types. In general, the specification specifies <i>applications</i> for clients to use when they themselves cannot operate on the specified MIME type. The
 * MailcapCommandMap extends this specification by using a parameter mechanism in mailcap files that allows JavaBeans(tm) components to be specified as corresponding to particular commands for a MIME
 * type.
 * <p>
 * When a mailcap file is parsed, the MailcapCommandMap recognizes certain parameter signatures, specifically those parameter names that begin with <code>x-java-</code>. The MailcapCommandMap uses
 * this signature to find command entries for inclusion into its registries. Parameter names with the form <code>x-java-&lt;name&gt;</code> are read by the MailcapCommandMap as identifying a command
 * with the name <i>name</i>. When the <i>name</i> is <code>
 * content-handler</code> the MailcapCommandMap recognizes the class signified by this parameter as a <i>DataContentHandler</i>. All other commands are handled generically regardless of command name.
 * The command implementation is specified by a fully qualified class name of a JavaBean(tm) component. For example; a command for viewing some data can be specified as:
 * <code>x-java-view=com.foo.ViewBean</code>.
 * <p>
 * When the command name is <code>fallback-entry</code>, the value of the command may be <code>true</code> or <code>false</code>. An entry for a MIME type that includes a parameter of
 * <code>x-java-fallback-entry=true</code> defines fallback commands for that MIME type that will only be used if no non-fallback entry can be found. For example, an entry of the form <code>text/*; ;
 * x-java-fallback-entry=true; x-java-view=com.sun.TextViewer</code> specifies a view command to be used for any text MIME type. This view command would only be used if a non-fallback view command for
...
public class MailcapCommandMap extends CommandMap {
	...
	/**
	 * Constructor that allows the caller to specify the path of a <i>mailcap</i> file.
	 *
	...
	/**
	 * Constructor that allows the caller to specify an <i>InputStream</i> containing a mailcap file.
	 *
	...
	/**
	 * Get the preferred command list for a MIME Type. The MailcapCommandMap searches the mailcap files as described above under <i>Mailcap file search order</i>.
	 * <p>
	 * The result of the search is a proper subset of available commands in all mailcap files known to this instance of MailcapCommandMap. The first entry for a particular command is considered the
	 * preferred command.
	 *
	...
	private DataContentHandler getDataContentHandler(String name) {
		...
		try {
			...
			if (cl != null) // XXX - always true?
				return (DataContentHandler) cl.getDeclaredConstructor().newInstance();
	...
	/**
	 * Get the native commands for the given MIME type. Returns an array of strings where each string is an entire mailcap file entry. The application will need to parse the entry to extract the
	 * actual command as well as any attributes it needs. See <A HREF="http://www.ietf.org/rfc/rfc1524.txt">RFC 1524</A> for details of the mailcap entry syntax. Only mailcap entries that specify a
	 * view command for the specified MIME type are returned.
	 *
	 * @param  mimeType the MIME type
	...
	/**
	 * for debugging... public static void main(String[] argv) throws Exception { MailcapCommandMap map = new MailcapCommandMap(); CommandInfo[] cmdInfo; cmdInfo = map.getPreferredCommands(argv[0]);
	 * System.out.println("Preferred Commands:"); for (int i = 0; i < cmdInfo.length; i++) System.out.println("Command " + cmdInfo[i].getCommandName() + " [" + cmdInfo[i].getCommandClass() + "]");
	 * cmdInfo = map.getAllCommands(argv[0]); System.out.println(); System.out.println("All Commands:"); for (int i = 0; i < cmdInfo.length; i++) System.out.println("Command " +
	 * cmdInfo[i].getCommandName() + " [" + cmdInfo[i].getCommandClass() + "]"); DataContentHandler dch = map.createDataContentHandler(argv[0]); if (dch != null) System.out.println("DataContentHandler
	 * " + dch.getClass().toString()); System.exit(0); }
	 */
}
