...
public class MimeTypeFile {
	...
	/**
	 * Appends string of entries to the types registry, must be valid .mime.types format. A mime.types entry is one of two forms: type/subtype ext1 ext2 ... or type=type/subtype desc="description of
	 * type" exts=ext1,ext2,... Example: # this is a test audio/basic au text/plain txt text type=application/postscript exts=ps,eps
	 */
	...
	// for debugging
	/*
	 * public static void main(String[] argv) throws Exception { MimeTypeFile mf = new MimeTypeFile(argv[0]); System.out.println("ext " + argv[1] + " type " + mf.getMIMETypeString(argv[1]));
	 * System.exit(0); }
	 */
}
...
class LineTokenizer {
	 ...
	 *
	 * @return <code>true</code> if there are more tokens available from this tokenizer's string; <code>false</code> otherwise.
	 */
	...
	public String nextToken() {
		...
		if (c == '"') {
			...
			while ((currentPosition < maxPosition) && singles.indexOf(str.charAt(currentPosition)) < 0 && !Character.isWhitespace(str.charAt(currentPosition))) {
				currentPosition++;
			}
		}
