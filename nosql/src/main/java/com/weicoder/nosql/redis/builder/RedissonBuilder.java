...
public final class RedissonBuilder {
	...
	public static RedissonClient newBuilder(String name) {
		...
		// 判断使用哪个模式
		if (EmptyUtil.isEmpty(RedisParams.getNodes(name))) {
			// 使用单机
			config.useSingleServer().setAddress(RedisParams.getAddress(name)).setPassword(RedisParams.getPassword(name))
			...
			// 设置节点
			for (String node : RedisParams.getNodes(name)) {
